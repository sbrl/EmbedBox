...
$action = str_replace(
	...
	ucwords(str_replace(
		"-", " ",
		preg_replace("/[^a-z-]/i", "", $_GET["action"] ?? $settings->get("routing.default_action"))
	))
);

$handler_name = "EmbedBox\\Actions\\$action";
...
$perfcounter->end("handle");

$result->header_set("x-perfdata", $perfcounter->render());
$result->render();
